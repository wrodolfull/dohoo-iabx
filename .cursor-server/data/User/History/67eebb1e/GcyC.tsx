...
const Surveys = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingSurvey) {
        await api.updateSurvey(editingSurvey.id.toString(), payload);
        toast.success('Pesquisa atualizada com sucesso!');
      } else {
        await api.createSurvey(tenantId, payload);
      ...
      toast.error(editingSurvey ? 'Erro ao atualizar pesquisa' : 'Erro ao criar pesquisa');
      console.error('Error saving survey:', error);
    }
  };

  const handleDelete = async (survey: Survey) => {
    if (!confirm(`Tem certeza que deseja excluir a pesquisa "${survey.name}"?`)) return;
    try {
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Select
                  ...
                  onChange={(option) => setFormData({ ...formData, survey_type: option?.value || 'dtmf' })}
                  options={surveyTypes}
