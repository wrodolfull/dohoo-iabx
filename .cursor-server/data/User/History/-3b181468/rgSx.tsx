...
const UserForm: React.FC<UserFormProps> = ({ 
  ...
  const [generateExtension, setGenerateExtension] = useState(true);
  const [createFreeSwitchUser, setCreateFreeSwitchUser] = useState(true);
  const [nextAvailableExtension, setNextAvailableExtension] = useState<string | null>(null);
  ...
  // Buscar próximo ramal disponível
  useEffect(() => {
    if (mode === 'create' && generateExtension) {
      fetchNextExtension();
      fetchAvailableExtensions();
    }
  }, [mode, generateExtension, tenantId]);

  const fetchNextExtension = async () => {
    if (!generateExtension) return;
    
    setLoadingExtension(true);
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://31.97.250.190:3001';
      const response = await fetch(`${apiUrl}/tenants/${tenantId}/next-extension`);
      
      if (response.ok) {
        const data = await response.json();
        setNextAvailableExtension(data.extension);
        setFormData(prev => ({ ...prev, extension: data.extension }));
      }
    } catch (error) {
      console.error('Erro ao buscar próximo ramal:', error);
      toast.error('Erro ao buscar próximo ramal disponível');
    } finally {
  ...
  const fetchAvailableExtensions = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || 'http://31.97.250.190:3001';
      const response = await fetch(`${apiUrl}/tenants/${tenantId}/available-extensions?limit=10`);
      ...
      console.error('Erro ao buscar ramais disponíveis:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...formData,
        tenant_id: tenantId,
        generateExtension: mode === 'create' ? generateExtension : false,
  ...
  const handleExtensionToggle = (checked: boolean) => {
    setGenerateExtension(checked);
    if (checked) {
      fetchNextExtension();
      fetchAvailableExtensions();
    } else {
      setFormData(prev => ({ ...prev, extension: '' }));
      setNextAvailableExtension(null);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {mode === 'create' 
              ? 'Preencha os dados para criar um novo usuário com ramal automático'
              ...
              {mode === 'create' && (
                <div className="flex items-center space-x-2">
                  <Switch
                    ...
                    checked={generateExtension}
                    onCheckedChange={handleExtensionToggle}
                  />
                  <Label htmlFor="generateExtension" className="text-sm">
                    Gerar ramal automaticamente
                  </Label>
                </div>
              )}

              {generateExtension && mode === 'create' ? (
                <div className="space-y-3">
                  <Label>Selecionar Ramal</Label>
                  
                  {loadingExtension ? (
                    <div className="flex items-center gap-2 text-blue-600">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span className="text-sm">Buscando ramais disponíveis...</span>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                          ...
                          Sugerido: Ramal {nextAvailableExtension}
                        </Badge>
                      </div>
                      
                      <div className="space-y-2">
                        <Label className="text-sm text-gray-600">Outras opções disponíveis:</Label>
                        <div className="flex flex-wrap gap-2">
                          {availableExtensions.slice(0, 8).map((ext) => (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setFormData(prev => ({ ...prev, extension: ext.toString() }));
                                setNextAvailableExtension(ext.toString());
                              }}
                              className={`px-3 py-1 text-sm rounded-md border transition-colors ${
                                formData.extension === ext.toString()
                          ...
                          {availableExtensions.length > 8 && (
                            <span className="text-xs text-gray-500 px-2 py-1">
                              +{availableExtensions.length - 8} mais
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        Não há ramais disponíveis na faixa configurada
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
