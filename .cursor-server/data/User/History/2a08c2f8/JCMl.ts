...
// Interceptor global para capturar todas as requisições
const originalFetch = window.fetch;
window.fetch = function(...args) {
  const url = args[0];
  if (typeof url === 'string' && url.includes('/tenants')) {
    console.log('🔍 Interceptor: Requisição para tenants detectada:', url);
    console.log('🔍 Interceptor: Método:', args[1]?.method || 'GET');
    console.log('🔍 Interceptor: Body:', args[1]?.body);
  }
...
export const api = {
  ...
  createTenant: (body: any) => {
    console.log('🔧 CreateTenant API call to:', `${API_URL}/tenants`);
    console.log('🔧 CreateTenant body:', body);
    return fetch(`${API_URL}/tenants`, { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify(body) 
    }).then(r => {
      console.log('🔧 CreateTenant response status:', r.status);
      return r.json();
    });
  },
