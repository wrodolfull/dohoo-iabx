...
export const api = {
  ...
  logout: () => 
    fetch(`${API_URL}/auth/logout`, { method: 'POST' }).then(r => r.json()),
  ...
  getRingGroupSurvey: (ringGroupId: string) =>
    fetch(`${API_URL}/ringgroups/${ringGroupId}/survey`).then(r => r.json()),
  updateRingGroupSurvey: (ringGroupId: string, body: any) =>
    fetch(`${API_URL}/ringgroups/${ringGroupId}/survey`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  getSurveys: (tenantId: string) => fetch(`${API_URL}/tenants/${tenantId}/surveys`).then(r => r.json()),
  createSurvey: (tenantId: string, body: any) => fetch(`${API_URL}/tenants/${tenantId}/surveys`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json()),
  updateSurvey: (id: string, body: any) => fetch(`${API_URL}/surveys/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }).then(r => r.json()),
  deleteSurvey: (id: string) => fetch(`${API_URL}/surveys/${id}`, { method: 'DELETE' }),
  ...
  // URA
  getURAs: (tenantId: string) => fetch(`${API_URL}/tenants/${tenantId}/ura`).then(r => r.json()),
  getURA: (id: string) => fetch(`${API_URL}/ura/${id}`).then(r => r.json()),
  ...
  provisionURA: (id: string) => 
    fetch(`${API_URL}/ura/${id}/provision-freeswitch`, { method: 'POST' }).then(r => r.json()),
  validateURAFlow: (flow: any) => 
    fetch(`${API_URL}/ura/validate-flow`, { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify({ flow }) 
    }).then(r => r.json()),
  getURATemplates: () => fetch(`${API_URL}/ura-templates`).then(r => r.json()),

  // TTS / ElevenLabs Integration
  getAudioFiles: (tenantId: string) => fetch(`${API_URL}/tenants/${tenantId}/audio-files`).then(r => r.json()),
  createAudioFile: (tenantId: string, body: any) => 
    fetch(`${API_URL}/tenants/${tenantId}/audio-files`, { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify(body) 
    }).then(r => r.json()),
  updateAudioFile: (id: string, body: any) => 
    fetch(`${API_URL}/audio-files/${id}`, { 
      method: 'PUT', 
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify(body) 
    }).then(r => r.json()),
  deleteAudioFile: (id: string) => fetch(`${API_URL}/audio-files/${id}`, { method: 'DELETE' }),
  generateTTSAudio: (tenantId: string, body: any) => 
    fetch(`${API_URL}/tenants/${tenantId}/generate-tts`, { 
      method: 'POST', 
      headers: { 'Content-Type': 'application/json' }, 
      body: JSON.stringify(body) 
    }).then(r => r.json()),
  getElevenLabsVoices: () => fetch(`${API_URL}/elevenlabs/voices`).then(r => r.json()),
  ...
  // Survey Reports
  getSurveyReports: (filters: any, tenantId: string) => {
    const params = new URLSearchParams();
    Object.keys(filters).forEach(key => {
    ...
    return fetch(`${API_URL}/survey-reports?${params}`, {
      headers: {
  ...
  // Test Survey Table
  testSurveyTable: () => fetch(`${API_URL}/test-survey-table`).then(r => r.json()),
