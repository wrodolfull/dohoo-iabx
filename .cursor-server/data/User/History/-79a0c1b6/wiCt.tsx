...
const SurveyReports = () => {
  ...
  const loadSurveyReports = async () => {
    ...
    try {
      ...
      const tableTest = await api.testSurveyTable();
      console.log('📋 Teste da tabela:', tableTest);
      
      if (!tableTest.tableExists) {
        console.log('❌ Tabela não existe, usando dados mock');
        // Usar dados mock se a tabela não existir
