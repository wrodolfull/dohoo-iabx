...
const Schedules = () => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      const [schedulesData, timeRangesData] = await Promise.all([
        api.getSchedules(tenantId),
        api.getTimeRanges(tenantId)
      ]);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSchedule) {
        ...
        await api.createSchedule(tenantId, formData);
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-gray-600">Escolha uma empresa para gerenciar seus horários</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {isSuperAdmin && (
        <TenantSelector
          ...
          description="Escolha a empresa cujos horários você deseja gerenciar"
        />
      )}
