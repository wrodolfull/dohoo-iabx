...
import { Menu, Phone, Users, Brain, Plus, Save, ArrowLeft, Loader2, Volume2, Clock, PhoneOff, Trash2, RotateCcw, Settings, CheckCircle, XCircle, ExternalLink, Edit } from "lucide-react";
import { toast } from "sonner";

// Componentes de nós com handles de conexão nas laterais
const MenuNode = ({ data, id, onDeleteNode }: any) => (
  <div className="bg-blue-100 border-2 border-blue-300 rounded-lg p-3 min-w-[220px] relative cursor-pointer hover:bg-blue-50 transition-colors group">
    <Handle
      type="target"
      ...
      <div className="mt-2 space-y-1">
        {data.options.map((opt: any, i: number) => (
          <div key={i} className="flex items-center gap-2 relative">
            <span className="text-xs text-blue-700 bg-blue-50 p-1 rounded min-w-[60px] text-center">
              {opt.key}: {opt.label}
            </span>
            <Handle
              type="source"
              ...
              id={`option-${i}`}
              className="w-3 h-3 bg-blue-500 border-2 border-white ml-2"
...
const ExtensionNode = ({ data, id, onDeleteNode }: any) => (
...
const GroupNode = ({ data, id, onDeleteNode }: any) => (
...
const TTSNode = ({ data, id, onDeleteNode }: any) => (
...
const ScheduleNode = ({ data, id, onDeleteNode }: any) => (
  <div className="bg-indigo-100 border-2 border-indigo-300 rounded-lg p-3 min-w-[220px] relative cursor-pointer hover:bg-indigo-50 transition-colors group">
    <Handle
      type="target"
      position={Position.Left}
    ...
    <div className="text-sm text-indigo-600 mt-1">{data.name || 'Verificação de Horário'}</div>
    <div className="text-xs text-indigo-700 mb-2">
      {data.schedule || 'Horário não configurado'}
    </div>
    ...
    <div className="flex items-center gap-2 mb-1">
      <Handle
        ...
        style={{ top: '60%' }}
      />
      <span className="text-xs text-green-700 font-medium">Dentro do Horário</span>
    </div>
    ...
    <div className="flex items-center gap-2">
      <Handle
        ...
        style={{ top: '80%' }}
      />
      <span className="text-xs text-red-700 font-medium">Fora do Horário</span>
    </div>
  </div>
);

const HangupNode = ({ data, id, onDeleteNode }: any) => (
...
const AINode = ({ data, id, onDeleteNode }: any) => (
  <div className="bg-yellow-100 border-2 border-yellow-300 rounded-lg p-3 min-w-[220px] relative cursor-pointer hover:bg-yellow-50 transition-colors group">
    <Handle
      type="target"
      position={Position.Left}
    ...
    <div className="font-semibold text-yellow-800 flex items-center gap-2">
      ...
      IA
      <div className="ml-auto text-xs text-yellow-600 opacity-0 group-hover:opacity-100 transition-opacity">
        Clique para editar
      </div>
      <button
        className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-red-600 hover:text-red-800"
    ...
    <div className="text-sm text-yellow-600 mt-1">{data.projectId || 'Projeto'}</div>
    <div className="text-xs text-yellow-700">{data.agentId || 'Agente'}</div>
    <Handle
      type="source"
      position={Position.Right}
      className="w-3 h-3 bg-yellow-500 border-2 border-white"
...
const StartNode = ({ data, id, onDeleteNode }: any) => (
  <div className="bg-red-100 border-2 border-red-300 rounded-lg p-3 min-w-[280px] relative cursor-pointer hover:bg-red-50 transition-colors group">
    {/* REMOVIDO: Handle de entrada (target) do lado esquerdo */}
    <div className="font-semibold text-red-800 flex items-center gap-2">
      <Phone className="w-4 h-4" />
      Início - Inbound Routes
      <div className="ml-auto text-xs text-red-600 opacity-0 group-hover:opacity-100 transition-opacity">
        Clique para editar
      </div>
      <button
    ...
    <div className="text-sm text-red-600 mt-1">
      {data.routesCount ? `${data.routesCount} rota(s) configurada(s)` : 'Nenhuma rota configurada'}
    </div>
    {data.routes && data.routes.length > 0 && (
      <div className="text-xs text-red-700 mt-1">
        DIDs: {data.routes.map((r: any) => r.did).join(', ')}
      </div>
    )}
    <div className="text-xs text-red-600 mt-1">
      {data.autoCreate ? 'Criação automática ativada' : 'Criação automática desativada'}
    </div>
    <Handle
      type="source"
      position={Position.Right}
...
// nodeTypes será definido dentro do componente URAEditor
...
const URAEditor = () => {
  ...
  const loadURA = async () => {
    try {
      ...
      if (uraData && !uraData.error) {
        ...
        // Verificar se existe um nó de início, se não existir, criar um
        setTimeout(() => ensureStartNode(), 100);
      } else {
  ...
  const ensureStartNode = () => {
    const hasStartNode = nodes.some(node => node.type === 'start');
    if (!hasStartNode) {
      addNode('start');
      toast.info('Nó de início adicionado automaticamente');
    }
  };
  ...
  const loadTenantData = async () => {
    try {
      ...
      // Carregar inbound routes
      const inboundRoutesData = await api.getInboundRoutes(currentTenantId);
      if (inboundRoutesData && !inboundRoutesData.error) {
        console.log('Inbound routes carregadas:', inboundRoutesData);
        setInboundRoutes(inboundRoutesData);
      }
  ...
  const addNode = (type: string) => {
    ...
    switch (type) {
      case 'start':
        data = { 
          routesCount: inboundRoutes.length,
          routes: inboundRoutes,
          autoCreate: true,
          defaultDid: '',
          defaultDestinationType: 'ura',
          defaultDestinationId: uraId || ''
        };
        break;
      case 'menu':
        data = { 
  ...
  const saveFlow = async () => {
    ...
    try {
      ...
      const validation = await api.validateURAFlow(flow);
      if (!validation.valid) {
      ...
      await api.provisionURA(uraId);
      toast.success('URA provisionada no FreeSWITCH!');
    } catch (error) {
      toast.error('Erro ao salvar ou provisionar URA');
      console.error('Error saving/provisioning URA:', error);
    } finally {
      setSaving(false);
    }
  };

  // Adicionar função handleDeleteNode no URAEditor
  const handleDeleteNode = (nodeId: string) => {
    setNodes(nodes.filter(node => node.id !== nodeId));
    setEdges(edges.filter(edge => edge.source !== nodeId && edge.target !== nodeId));
  };

  // Definir nodeTypes dentro do componente para ter acesso ao handleDeleteNode
  const nodeTypes: NodeTypes = useMemo(() => ({
    start: (props) => <StartNode {...props} onDeleteNode={handleDeleteNode} />,
    menu: (props) => <MenuNode {...props} onDeleteNode={handleDeleteNode} />,
    extension: (props) => <ExtensionNode {...props} onDeleteNode={handleDeleteNode} />,
    group: (props) => <GroupNode {...props} onDeleteNode={handleDeleteNode} />,
    tts: (props) => <TTSNode {...props} onDeleteNode={handleDeleteNode} />,
    schedule: (props) => <ScheduleNode {...props} onDeleteNode={handleDeleteNode} />,
    hangup: (props) => <HangupNode {...props} onDeleteNode={handleDeleteNode} />,
    ai: (props) => <AINode {...props} onDeleteNode={handleDeleteNode} />,
  }), [handleDeleteNode]);
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="max-w-md" aria-describedby="node-edit-description">
          <DialogHeader>
            <DialogTitle>Editar {editingNode?.type?.toUpperCase()}</DialogTitle>
          </DialogHeader>
          <div id="node-edit-description" className="sr-only">
            Modal para editar configurações do nó selecionado
          </div>
          ...
          {editingNode && (
            <div className="space-y-4">
              {editingNode.type === 'start' && (
                <div className="space-y-4">
                  <div>
                    <Label className="text-sm font-medium">Rotas Inbound Configuradas</Label>
                    <div className="mt-2 space-y-2 max-h-40 overflow-y-auto">
                      {inboundRoutes.length > 0 ? (
                        inboundRoutes.map((route: any) => (
                          <div key={route.id} className="flex items-center justify-between p-2 bg-gray-50 rounded border">
                            <div>
                              <div className="font-medium text-sm">{route.did}</div>
                        ...
                        <div className="text-center py-4 text-gray-500">
                          ...
                          <p className="text-sm">Nenhuma rota inbound configurada</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      id="autoCreate"
                      checked={Boolean(editingNode.data?.autoCreate) || false}
                      onChange={(e) => updateNode({ autoCreate: e.target.checked })}
                      className="rounded"
                    />
                    <Label htmlFor="autoCreate">Criar rota automaticamente se não existir</Label>
                  </div>

                  {editingNode.data?.autoCreate && (
                    <div className="space-y-3 p-3 bg-blue-50 rounded border">
                      <Label className="text-sm font-medium text-blue-800">Configuração de Rota Automática</Label>
                      
                      <div>
                        <Label htmlFor="defaultDid">DID Padrão</Label>
                        <Input
                          id="defaultDid"
                          value={String(editingNode.data?.defaultDid || '')}
                          onChange={(e) => updateNode({ defaultDid: e.target.value })}
                          placeholder="Ex: +5511999999999"
                        />
                      </div>

                      <div>
                        <Label htmlFor="defaultDestinationType">Tipo de Destino</Label>
                        <Select
                          value={String(editingNode.data?.defaultDestinationType || 'ura')}
                          onValueChange={(value) => updateNode({ defaultDestinationType: value })}
                        >
                          <SelectTrigger>
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="ringgroup">Grupo de Toque</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>

                      <div>
                        <Label htmlFor="defaultDestinationId">Destino</Label>
                        <Select
                          value={String(editingNode.data?.defaultDestinationId || '')}
                          onValueChange={(value) => updateNode({ defaultDestinationId: value })}
                        >
                          <SelectTrigger>
                            <SelectValue placeholder="Selecione o destino" />
                          </SelectTrigger>
                          <SelectContent>
                            {editingNode.data?.defaultDestinationType === 'ura' && (
                              <SelectItem value={uraId || ''}>Esta URA</SelectItem>
                            )}
                            {editingNode.data?.defaultDestinationType === 'extension' && 
                            ...
                            {editingNode.data?.defaultDestinationType === 'ringgroup' && 
                              ringGroups.map((group: any) => (
                      ...
                      <Button
                        onClick={async () => {
                          const currentTenantId = selectedTenantId || tenantId;
                          if (!currentTenantId || !editingNode.data?.defaultDid || !editingNode.data?.defaultDestinationId) {
                          ...
                          try {
                            const routeData = {
                              did: editingNode.data.defaultDid,
                              destination_type: editingNode.data.defaultDestinationType,
                              destination_id: editingNode.data.defaultDestinationId,
                              caller_id_name: `URA ${uraName}`
                            };

                            const result = await api.createInboundRoute(currentTenantId, routeData);
                            if (result && !result.error) {
                              toast.success('Rota inbound criada com sucesso!');
                              // Recarregar as rotas
                              const newRoutes = await api.getInboundRoutes(currentTenantId);
                              if (newRoutes && !newRoutes.error) {
                                setInboundRoutes(newRoutes);
                                updateNode({ 
                                  routesCount: newRoutes.length,
                                  routes: newRoutes
                                });
                              }
                            } else {
                              toast.error('Erro ao criar rota inbound');
                            }
                          } catch (error) {
                        ...
                        className="w-full"
                        disabled={!editingNode.data?.defaultDid || !editingNode.data?.defaultDestinationId}
                      >
              ...
              {editingNode.type === 'extension' && (
                <div className="space-y-3">
                  <div>
                    <Label htmlFor="number">Selecionar Ramal</Label>
                    <Select
                      value={String(editingNode.data?.number || '')}
                      onValueChange={(value) => {
                        const selectedExt = extensions.find(ext => String(ext.number) === String(value));
                        updateNode({
                          number: value,
                    ...
                    >
                      ...
                      <SelectContent>
                        {extensions.length > 0 ? (
                          extensions.map((ext) => (
                            <SelectItem key={ext.id} value={String(ext.number)}>
                              {ext.number} - {ext.name}
                            </SelectItem>
                          ))
                        ) : (
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        const selectedGroup = ringGroups.find(group => String(group.id) === String(value));
                        updateNode({ 
                          groupId: value,
              ...
              {editingNode.type === 'schedule' && (
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="schedule-name">Nome da Verificação</Label>
                    <Input
                      id="schedule-name"
                      value={(editingNode.data?.name as string) || ''}
                      onChange={(e) => updateNode({ name: e.target.value })}
                      ...
                      {[
                        ...
                        <div key={day.key} className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id={day.key}
                            checked={editingNode.data?.daysOfWeek?.[day.key] || false}
                            onChange={(e) => {
                              const currentDays = editingNode.data?.daysOfWeek || {};
                              updateNode({
                                daysOfWeek: {
                                  ...currentDays,
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {((editingNode.data?.timeRanges as any[]) || [{ start: '09:00', end: '18:00' }]).map((range: any, index: number) => (
                        <div key={index} className="flex items-center gap-2">
                          <Input
                            type="time"
                            value={range.start || '09:00'}
                            onChange={(e) => {
                              const currentRanges = (editingNode.data?.timeRanges as any[]) || [{ start: '09:00', end: '18:00' }];
                              const newRanges = [...currentRanges];
                              newRanges[index] = { ...(newRanges[index] as any), start: e.target.value };
                              updateNode({ timeRanges: newRanges });
                            }}
                            className="w-24"
                          />
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              const currentRanges = (editingNode.data?.timeRanges as any[]) || [{ start: '09:00', end: '18:00' }];
                              const newRanges = [...currentRanges];
                              newRanges[index] = { ...(newRanges[index] as any), end: e.target.value };
                              updateNode({ timeRanges: newRanges });
                            }}
                            className="w-24"
                          />
                          {((editingNode.data?.timeRanges as any[]) || []).length > 1 && (
                            <Button
                              type="button"
                              variant="outline"
                              size="sm"
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const currentRanges = (editingNode.data?.timeRanges as any[]) || [{ start: '09:00', end: '18:00' }];
                          updateNode({ timeRanges: [...currentRanges, { start: '09:00', end: '18:00' }] });
                        }}
                        className="w-full"
                      >
                        <Plus className="w-3 h-3 mr-1" />
                  ...
                  {/* Feriados */}
                  <div>
                    <Label className="text-sm font-medium">Feriados (Opcional)</Label>
                    <div className="space-y-2 mt-2">
                      {((editingNode.data?.holidays as any[]) || []).map((holiday: any, index: number) => (
                        <div key={index} className="flex items-center gap-2">
                          <Input
                            type="date"
                            value={holiday.date || ''}
                            onChange={(e) => {
                              const currentHolidays = (editingNode.data?.holidays as any[]) || [];
                              const newHolidays = [...currentHolidays];
                              newHolidays[index] = { ...(newHolidays[index] as any), date: e.target.value };
                              updateNode({ holidays: newHolidays });
                            }}
                            className="flex-1"
                          />
                          <Input
                            ...
                            onChange={(e) => {
                              const currentHolidays = (editingNode.data?.holidays as any[]) || [];
                              const newHolidays = [...currentHolidays];
                              newHolidays[index] = { ...(newHolidays[index] as any), description: e.target.value };
                              updateNode({ holidays: newHolidays });
                            }}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              const currentHolidays = (editingNode.data?.holidays as any[]) || [];
                              const newHolidays = currentHolidays.filter((_: any, i: number) => i !== index);
                              updateNode({ holidays: newHolidays });
                            }}
                            className="w-8 h-8 p-0"
                          >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const currentHolidays = (editingNode.data?.holidays as any[]) || [];
                          updateNode({ holidays: [...currentHolidays, { date: '', description: '' }] });
                        }}
                        className="w-full"
                      >
                        <Plus className="w-3 h-3 mr-1" />
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    <Label className="text-sm font-medium text-gray-700">Resumo da Configuração</Label>
                    <div className="text-xs text-gray-600 mt-1">
                      <div>Dias: {Object.entries((editingNode.data?.daysOfWeek as Record<string, boolean>) || {}).filter(([_, checked]) => checked).map(([day, _]) => day).join(', ') || 'Nenhum dia selecionado'}</div>
                      <div>Horários: {((editingNode.data?.timeRanges as any[]) || [{ start: '09:00', end: '18:00' }]).map((range: any) => `${range.start}-${range.end}`).join(', ')}</div>
                      <div>Feriados: {((editingNode.data?.holidays as any[]) || []).length} configurado(s)</div>
                    </div>
                  </div>
                </div>
              )}
