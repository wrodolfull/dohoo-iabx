              ...
              {editingNode.type === 'schedule' && (
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {(editingNode.data?.holidays || []).map((holiday: any, index: number) => (
                        <div key={index} className="flex items-center gap-2">
                          <Input
                            type="date"
                            value={holiday.date || ''}
                            onChange={(e) => {
                              const currentHolidays = editingNode.data?.holidays || [];
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg">
                    ...
                    <div className="text-xs text-gray-600 mt-1">
                      <div>Dias: {Object.entries(editingNode.data?.daysOfWeek || {}).filter(([_, checked]) => checked).map(([day, _]) => day).join(', ') || 'Nenhum dia selecionado'}</div>
                      <div>Horários: {(editingNode.data?.timeRanges || [{ start: '09:00', end: '18:00' }]).map((range: any) => `${range.start}-${range.end}`).join(', ')}</div>
                      <div>Feriados: {(editingNode.data?.holidays || []).length} configurado(s)</div>
                    </div>
                  </div>
                </div>
              )}
