...
const Trunks = () => {
  ...
  const loadTrunks = async () => {
    ...
    try {
      setLoading(true);
      const data = await api.getTrunks(selectedTenant);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTrunk) {
        ...
        await api.createTrunk(selectedTenant, formData);
      ...
      {user?.role === 'superadmin' && tenants.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Escolha a empresa para gerenciar os troncos SIP
            </CardDescription>
          </CardHeader>
          <CardContent>
            <select 
        ...
        <CardContent>
          {filteredTrunks.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {selectedTenant ? 'Nenhum tronco encontrado' : 'Selecione uma empresa para visualizar os troncos'}
            </div>
          ) : (
            <Table>
              <TableHeader>
