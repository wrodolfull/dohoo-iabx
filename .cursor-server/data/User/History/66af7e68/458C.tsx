...
import { Search, Plus, Edit, Trash2, Volume2, Play, Settings, Loader2, CheckCircle, XCircle, Mic, Headphones, ExternalLink } from "lucide-react";
...
const URABuilder = () => {
  ...
  const handleProvisionURA = async (ura: URA) => {
    try {
      const result = await api.provisionURAInFreeSWITCH(ura.id);
      if (result?.error) throw new Error(result.error);
