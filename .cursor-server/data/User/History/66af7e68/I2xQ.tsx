...
import { Search, Plus, Edit, Trash2, Volume2, Play, Settings, Loader2, CheckCircle, XCircle, Mic, Headphones } from "lucide-react";
...
const URABuilder = () => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true);
      const urasData = await api.getURA(tenantId);
      setUras(Array.isArray(urasData) ? urasData : []);
      // TODO: Implementar API para carregar arquivos de áudio
      setAudioFiles([]);
    } catch (error) {
      toast.error('Erro ao carregar URAs');
      console.error('Error loading URAs:', error);
      setUras([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSaveURA = async () => {
    ...
    try {
      const payload = {
        ...
        flow: editingURA?.flow || { nodes: [], edges: [] }
      };
  ...
  const handleGenerateAudio = async () => {
    ...
    try {
      // TODO: Implementar API para gerar áudio com ElevenLabs
      const payload = {
  ...
  const handleProvisionURA = async (ura: URA) => {
    try {
      // TODO: Implementar provisionamento no FreeSWITCH
          ...
          <Dialog open={isURADialogOpen} onOpenChange={setIsURADialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleSaveURA} disabled={!formData.name}>
