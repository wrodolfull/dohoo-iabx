...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const [extensions, ringGroups, trunks, activeCalls, callReports, fsStatus] = await Promise.all([
        api.getExtensions(tenantId),
        api.getRingGroups(tenantId),
        api.getTrunks(tenantId),
        api.getActiveCalls(tenantId),
        api.getCallReports(tenantId, {
      ...
      console.log('RG', ringGroups);
      console.log('TRUNKS', trunks);
      console.log('ACTIVE', activeCalls);
      console.log('CALLREPORTS', callReports);
      console.log('FSSTATUS', fsStatus);

      // Processar estatísticas de chamadas
      let totalCalls = 0, received = 0, missed = 0, avgDuration = 0;
      if (Array.isArray(callReports)) {
        totalCalls = callReports.length;
        received = callReports.filter((c: any) => c.status === 'answered').length;
        missed = callReports.filter((c: any) => c.status === 'missed').length;
        const totalDuration = callReports.reduce((sum: number, c: any) => sum + (c.duration || 0), 0);
        avgDuration = received > 0 ? totalDuration / received : 0;
      }
