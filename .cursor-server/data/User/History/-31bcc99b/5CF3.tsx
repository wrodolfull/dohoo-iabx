...
const Extensions = () => {
  ...
  // Corrigir o tipo de formData para incluir sip_password
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingExtension) {
        // Permitir editar a senha apenas na edição
        if (typeof formData.sip_password === 'string' && formData.sip_password.trim()) {
          payload.sip_password = formData.sip_password;
        }
        await api.updateExtension(editingExtension.id, payload);
        toast.success('Ramal atualizado com sucesso!');
      } else {
        await api.createExtension(tenantId, payload);
