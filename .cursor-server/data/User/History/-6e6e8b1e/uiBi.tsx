  ...
  const getUserFromDatabase = async (email: string) => {
    try {
      ...
      return {
        ...
        email: data.email,
        role: data.role,
        company: Array.isArray(data.tenants) ? data.tenants[0]?.name || 'Empresa' : data.tenants?.name || 'Empresa',
        tenant_id: data.tenant_id,
        domain: Array.isArray(data.tenants) ? data.tenants[0]?.domain || 'localhost' : data.tenants?.domain || 'localhost',
        permissions: ROLE_PERMISSIONS[data.role] || ROLE_PERMISSIONS.agent
      };
    } catch (error) {
