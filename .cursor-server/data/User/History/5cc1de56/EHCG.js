...
app.post('/tenants/:tenantId/surveys', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    try {
      ...
      await provisionSurvey({ ...createdSurvey, tenant_id: tenant.id }, tenant);
...
app.put('/surveys/:id', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    try {
      ...
      await provisionSurvey({ ...updatedSurvey, tenant_id: tenant.id }, tenant);
...
app.delete('/surveys/:id', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    try {
      await removeSurveyFromFreeSWITCH(id, tenant);
...
// === Teste de Tabela de Pesquisas ===
app.get('/test-survey-table', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    if (tableError) {
      return res.status(500).json({ error: 'Erro ao verificar tabela', details: tableError });
    }

    if (!tableExists || tableExists.length === 0) {
      return res.json({ 
        tableExists: false, 
        message: 'Tabela cc_survey_responses não existe. Execute o SQL no Supabase primeiro.' 
      });
    }
    ...
    if (dataError) {
      return res.status(500).json({ error: 'Erro ao buscar dados', details: dataError });
    }

    res.json({
      tableExists: true,
      dataCount: responses.length,
...
app.get('/survey-reports', checkSupabaseConnection, async (req, res) => {
  try {
    const { startDate, endDate, ringGroup, rating } = req.query;
    ...
    let query = supabase
      ...
      `)
      .eq('ringgroups.tenant_id', tenantId);
    ...
    if (ringGroup && ringGroup !== 'all') {
      query = query.eq('ringgroups.name', ringGroup);
    }
    ...
    if (error) {
      console.error('Erro ao buscar respostas de pesquisa:', error);
      return res.status(500).json({ error: error.message });
    }
    ...
    const satisfactionRate = totalResponses > 0 ? 
      (responses.filter(r => r.rating >= 4).length / totalResponses) * 100 : 0;

    // Contar avaliações por rating
    const ratings = {};
    for (let i = 1; i <= 5; i++) {
      ratings[i] = responses.filter(r => r.rating === i).length;
    }
    ...
    // Estatísticas por ring group
    const ringGroupStats = {};
    responses.forEach(response => {
      const groupName = response.ringgroups.name;
      if (!ringGroupStats[groupName]) {
        ringGroupStats[groupName] = { total: 0, average: 0, responses: 0 };
      }
      ringGroupStats[groupName].total += response.rating;
      ringGroupStats[groupName].responses += 1;
    });

    // Calcular médias por ring group
    ...
    // Buscar total de chamadas para calcular taxa de resposta
    const { data: callStats, error: callError } = await supabase
      .from('cc_calls')
      .select('id')
      .eq('tenant_id', tenantId)
      .gte('start_time', startDate ? `${startDate}T00:00:00` : '1970-01-01T00:00:00')
      .lte('start_time', endDate ? `${endDate}T23:59:59` : new Date().toISOString());

    if (callError) {
      console.error('Erro ao buscar estatísticas de chamadas:', callError);
    }
    ...
    console.error('Erro interno ao gerar relatório de pesquisa:', error);
    res.status(500).json({ error: error.message });
  }
});