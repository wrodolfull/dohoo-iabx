...
app.post('/tenants/:tenantId/extensions', checkSupabaseConnection, async (req, res) => {
  try {
    const { tenantId } = req.params;
    const { name, number, is_active, created_by, sip_password } = req.body;
    ...
    const { data, error } = await supabase.from('extensions').insert([
      { tenant_id: tenantId, name, number, sip_password, is_active, created_by }
    ]).select();
    if (error) {
...
app.put('/extensions/:id', checkSupabaseConnection, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, number, sip_password, is_active } = req.body;
    
    const { data, error } = await supabase
      .from('extensions')
      .update({ name, number, sip_password, is_active })
      .eq('id', id)
      .select();
...
function provisionRingGroup(ringGroup, extensions) {
    ...
    if (flow !== undefined) {
      try {
        ...
        if (!tenantError && tenant) {
          ...
          // Salvar arquivo de dialplan
          const context = tenant.context || `context_${tenant.name.toLowerCase().replace(/\s+/g, '_')}`;
          const dialplanPath = `/usr/local/freeswitch/conf/dialplan/${context}_ura_${data[0].id}.xml`;
          await writeFreeSWITCHConfig(dialplanPath, dialplanXml);
          
          // Recarregar configuração do FreeSWITCH
          await reloadFreeSWITCHConfig();
          
          console.log(`✅ URA ${data[0].name} provisionada automaticamente no FreeSWITCH`);
        }
...
app.post('/ura/:id/provision-freeswitch', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    // Salvar arquivo de dialplan
    const context = tenant.context || `context_${tenant.name.toLowerCase().replace(/\s+/g, '_')}`;
    const dialplanPath = `/usr/local/freeswitch/conf/dialplan/${context}_ura_${ura.id}.xml`;
    await writeFreeSWITCHConfig(dialplanPath, dialplanXml);
...
async function getNextAvailableExtension(tenantId) {
  try {
    ...
    // TODO: Implementar quando a coluna extension for adicionada à tabela users
    // Temporariamente retornando um número fixo para evitar erro
    console.log(`⚠️ Verificação de ramais utilizados desabilitada temporariamente`);
    const used = [];
    
    // Encontrar primeiro número disponível na faixa
    for (let ext = startRange; ext <= endRange; ext++) {
...
app.post('/tenants/:tenantId/users', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    // Auto-gerar ramal se solicitado
    if (generateExtension && !userExtension) {
      try {
        userExtension = await getNextAvailableExtension(tenantId);
      } catch (extError) {
        return res.status(400).json({ 
          error: 'Erro ao gerar ramal: ' + extError.message 
        });
      }
    }
    
    // Verificar se ramal já está em uso (se especificado)
    // TODO: Implementar verificação de ramal quando a coluna extension for adicionada à tabela users
    if (userExtension) {
      // Temporariamente desabilitado até a coluna extension ser adicionada
      console.log(`⚠️ Verificação de ramal ${userExtension} desabilitada temporariamente`);
    }
    ...
    try {
      ...
      // 4. CRIAR RAMAL NO FREESWITCH (se extension fornecida)
      if (userExtension && createFreeSwitchUser) {
        try {
...
app.put('/users/:id', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    if (extension && extension !== currentUser.extension) {
      // Temporariamente desabilitado até a coluna extension ser adicionada
      console.log(`⚠️ Verificação de ramal ${extension} desabilitada temporariamente`);
    }
...
function generateScheduleNodeXml(node, index, edges, context) {
  ...
  if (selectedDays.length > 0) {
    xml += `        <action application="set" data="current_day=${new Date().getDay()}"/>\n`;
