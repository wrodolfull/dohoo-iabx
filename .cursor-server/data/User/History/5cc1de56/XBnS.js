...
app.get('/survey-reports', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    let query = supabase
      ...
      .select(`
        *,
        cc_post_call_survey!inner(
          ring_group_id,
          cc_ring_groups!inner(
            name,
            tenant_id
          )
        )
      `)
      .eq('cc_post_call_survey.cc_ring_groups.tenant_id', tenantId);
    ...
    if (ringGroup && ringGroup !== 'all') {
      query = query.eq('cc_post_call_survey.cc_ring_groups.name', ringGroup);
    }
    ...
    responses.forEach(response => {
      const groupName = response.cc_post_call_survey.cc_ring_groups.name;
    ...
    res.json({
      responses: responses.map(r => ({
        ...
        ring_group: r.cc_post_call_survey.cc_ring_groups.name,
        comments: r.comments
      })),
      stats: {
        totalResponses,
