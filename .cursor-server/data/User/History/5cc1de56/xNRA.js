...
async function provisionRingGroup(ringGroup, extensions) {
  try {
    // Buscar configuração da pesquisa de satisfação
    ...
    if (surveyConfig && surveyConfig.enabled) {
      // Mensagem de pesquisa (pode ser TTS ou arquivo .wav)
      dialplanXml += `    <action application=\"playback\" data=\"survey_prompt.wav\"/>\n`;
      dialplanXml += `    <action application=\"read\" data=\"1 1 5 3000 # survey_response\"/>\n`;
      dialplanXml += `    <action application=\"curl\" data=\"https://SUA_API/survey_response?call_id=${'${uuid}'}&ring_group_id=${ringGroup.id}&response=${'${survey_response}'}\"/>\n`;
    }
...
// GET configuração da pesquisa de satisfação de um ring group
...
// PUT para atualizar/criar configuração da pesquisa de satisfação
app.put('/ringgroups/:id/survey', checkSupabaseConnection, async (req, res) => {
  try {
    const { id } = req.params;
    const { enabled, survey_type, question_text } = req.body;
    ...
    if (existing) {
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          survey_type: survey_type || existing.survey_type,
        ...
        .insert([
          {
            ...
            question_text: question_text || 'Como você avalia seu atendimento? (1 a 5)',
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          },
        ])
