...
async function provisionSurvey(survey, tenant) {
  try {
    ...
    // Caminho da pasta do tenant
    const tenantDir = `/usr/local/freeswitch/conf/surveys/${survey.tenant_id}`;
    if (!fs.existsSync(tenantDir)) {
      fs.mkdirSync(tenantDir, { recursive: true });
      console.log(`✅ Pasta criada para tenant: ${tenantDir}`);
    }
    // Escrever arquivo de configuração
    const surveyConfigPath = `${tenantDir}/survey_${survey.id}.xml`;
    await writeFreeSWITCHConfig(surveyConfigPath, surveyXml);
    
    console.log(`✅ Pesquisa ${survey.name} provisionada com sucesso no FreeSWITCH (tenant: ${survey.tenant_id})`);
...
async function removeSurveyFromFreeSWITCH(surveyId, tenantId) {
  try {
    console.log(`🗑️ [Provision] Removendo pesquisa ID ${surveyId} do FreeSWITCH`);
    const tenantDir = `/usr/local/freeswitch/conf/surveys/${tenantId}`;
    const surveyConfigPath = `${tenantDir}/survey_${surveyId}.xml`;
    // Verificar se arquivo existe antes de tentar remover
    if (fs.existsSync(surveyConfigPath)) {
      fs.unlinkSync(surveyConfigPath);
      console.log(`✅ Arquivo de configuração da pesquisa ${surveyId} removido de ${tenantDir}`);
    }
...
app.post('/tenants/:tenantId/surveys', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    try {
      await provisionSurvey(createdSurvey, tenant);
...
app.put('/surveys/:id', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    // Provisionar automaticamente no FreeSWITCH
    try {
      await provisionSurvey(updatedSurvey, tenant);
