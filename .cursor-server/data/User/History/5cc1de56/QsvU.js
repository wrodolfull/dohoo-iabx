...
async function provisionSurvey(survey, tenant) {
  try {
    ...
    surveyXml += `<survey id="${survey.id}" name="${survey.name}" type="${survey.survey_type}">\n`;
    ...
    const surveyConfigPath = `/usr/local/freeswitch/conf/surveys/survey_${survey.id}.xml`;
    await writeFreeSWITCHConfig(surveyConfigPath, surveyXml);
    
    console.log(`✅ Pesquisa ${survey.name} provisionada com sucesso no FreeSWITCH`);
...
async function removeSurveyFromFreeSWITCH(surveyId) {
  try {
    console.log(`🗑️ [Provision] Removendo pesquisa ID ${surveyId} do FreeSWITCH`);
    
    const surveyConfigPath = `/usr/local/freeswitch/conf/surveys/survey_${surveyId}.xml`;
    
    // Verificar se arquivo existe antes de tentar remover
    if (fs.existsSync(surveyConfigPath)) {
      fs.unlinkSync(surveyConfigPath);
      console.log(`✅ Arquivo de configuração da pesquisa ${surveyId} removido`);
    }
...
app.delete('/surveys/:id', checkSupabaseConnection, async (req, res) => {
  try {
    ...
    try {
      await removeSurveyFromFreeSWITCH(id);
