...
const RingGroups = () => {
  ...
  const handleSave = async () => {
    if (!tenantId) return;
    try {
      // Filtrar apenas os campos válidos para o backend
      const { number, ...validFormData } = formData;
      
      if (editingRingGroup) {
        await api.updateRingGroup(editingRingGroup.id, validFormData);
        toast.success('Grupo de toque atualizado com sucesso!');
      } else {
        await api.createRingGroup(tenantId, validFormData);
