...
const RingGroups = () => {
  ...
  const [surveyConfig, setSurveyConfig] = useState({
    enabled: false,
    question_text: 'Como você avalia seu atendimento? (1 a 5)'
  });
  ...
  useEffect(() => {
    if (editingRingGroup) {
      ...
      api.get(`/ringgroups/${editingRingGroup.id}/survey`).then((data) => {
        if (data) setSurveyConfig({
          enabled: !!data.enabled,
          question_text: data.question_text || 'Como você avalia seu atendimento? (1 a 5)'
        });
        else setSurveyConfig({ enabled: false, question_text: 'Como você avalia seu atendimento? (1 a 5)' });
      }).finally(() => setSurveyLoading(false));
    } else {
      setSurveyConfig({ enabled: false, question_text: 'Como você avalia seu atendimento? (1 a 5)' });
    }
  }, [editingRingGroup]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editingRingGroup) {
        ...
        await api.put(`/ringgroups/${editingRingGroup.id}/survey`, surveyConfig);
        toast.success('Grupo de toque atualizado com sucesso!');
      } else {
        const newGroup = await api.createRingGroup(tenantId, payload);
        // Salvar config pesquisa
        if (newGroup?.id) await api.put(`/ringgroups/${newGroup.id}/survey`, surveyConfig);
          ...
          <DialogContent className="max-h-[80vh] overflow-y-auto w-full max-w-lg mx-auto">
            ...
            <div className="space-y-4">
              ...
              {surveyConfig.enabled && (
                <div className="mt-2">
                  <Label htmlFor="survey_question">Mensagem/pergunta da pesquisa</Label>
                  <Input
                    id="survey_question"
                    value={surveyConfig.question_text}
                    onChange={e => setSurveyConfig(s => ({ ...s, question_text: e.target.value }))}
