...
const RingGroups = () => {
  ...
  const loadRingGroups = async () => {
    ...
    try {
      setLoading(true);
      const data = await api.getRingGroups(selectedTenant);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingRingGroup) {
        await api.updateRingGroup(editingRingGroup.id, formData);
        toast.success('Grupo de toque atualizado com sucesso!');
      } else {
        await api.createRingGroup(selectedTenant, formData);
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="strategy">Estratégia de Toque</Label>
                <select
                  id="strategy"
      ...
      {user?.role === 'superadmin' && tenants.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Escolha a empresa para gerenciar os grupos de toque
            </CardDescription>
          </CardHeader>
          <CardContent>
            <select 
        ...
        <CardContent>
          {filteredRingGroups.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {selectedTenant ? 'Nenhum grupo de toque encontrado' : 'Selecione uma empresa para visualizar os grupos'}
            </div>
          ) : (
            <Table>
              <TableHeader>
