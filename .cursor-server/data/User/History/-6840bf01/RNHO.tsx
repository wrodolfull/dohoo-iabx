...
export function Header() {
  ...
  const { isSuperAdmin, selectedTenantId, handleTenantChange } = useTenant();
    ...
    <header className="h-16 border-b bg-white/80 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-40">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <Button variant="ghost" size="icon" className={theme === 'light' ? 'text-yellow-500' : theme === 'dark' ? 'text-blue-400' : 'text-gray-600'} onClick={() => setTheme(theme === 'light' ? 'dark' : theme === 'dark' ? 'system' : 'light')} title={theme === 'light' ? 'Tema claro' : theme === 'dark' ? 'Tema escuro' : 'Tema do sistema'}>
